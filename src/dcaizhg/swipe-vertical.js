...
function SwipeVertical(container, options) {
  ...
  function setup() {
	...
	//height = container.getBoundingClientRect().height || container.offsetheight || (typeof(jQuery) === "function" ? jQuery(container).height() : undefined);
  ...
  var events = {
    ...
    move: function(event) {
      ...
      if ( typeof isScrolling == 'undefined') {
        isScrolling = !!( isScrolling || Math.abs(delta.y) < Math.abs(delta.y) );
      }
      ...
      // determine direction of swipe (true:right, false:top)
...
if ( window.jQuery || window.Zepto ) {
  (function($) {
    $.fn.SwipeVertical = function(params) {
      return this.each(function() {
        $(this).data('SwipeVertical', new SwipeVertical($(this)[0], params));
      });
    }
